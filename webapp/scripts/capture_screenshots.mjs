...
const outDir2 =
  "C:/Users/kkchi/.gemini/antigravity/brain/a27e01f8-9736-4e28-8853-b2409635b305/screenshots";
...
async function capture() {
    ...
    if (url.includes("exams")) {
      return route.fulfill({
        ...
        body: JSON.stringify([
          {
            id: 1,
            exam_name: "Advanced Calculus Midterm",
          ...
          {
            id: 2,
            exam_name: "Quantum Physics Final",
    ...
    if (url.includes("quiz_attempts")) {
      return route.fulfill({
        ...
        body: JSON.stringify([
          {
            id: 1,
            quiz_title: "Calculus Derivatives",
            score: 72,
            total_questions: 10,
            created_at: "2026-08-25T10:00:00Z",
            weak_topics: [
              "Chain Rule of Composites",
  ...
  await page.goto("http://localhost:5199/app/login", {
  ...
  await page.evaluate(() => {
    ...
    const mockTrace = {
      ...
      failedQuestionOrTopic:
        "Failed derivative of composite trigonometric function sin(x^2)",
      userAttemptContext:
        "Calculated cos(x^2) and missed multiplying by the inner derivative 2x.",
      rootCauseSummary:
        "Surface breakdown occurred because the core prerequisite invariant of Function Composition & Inner Rate Scaling (Chain Rule Bedrock) was skipped.",
      ...
      layers: [
        {
          ...
          status: "severed",
          explanation:
            "Foundational bedrock: Rates of nested functions compound multiplicatively rather than additively.",
        ...
        },
        {
          ...
          status: "shaky",
          explanation:
            "Intermediate bridge: Identifying u = g(x) substitution and computing df/du * du/dx.",
        ...
        },
        {
          ...
          status: "severed",
          explanation:
            "Surface problem: Evaluating d/dx[sin(x^2)] in exam problems.",
        },
      ],
    };
    ...
    const mockFeynman = {
      ...
      draft: {
        draftText:
          "Photosynthesis happens when plants absorb sunlight directly into chloroplasts to create energy. The chlorophyll molecules eat light photons and immediately turn them into glucose sugar in one quick step, releasing water as a byproduct.",
        hiddenMisconceptions: [
          {
            id: "misc-1",
            concept: "Two-Stage Separation (Light vs Calvin Cycle)",
            snippet:
              "immediately turn them into glucose sugar in one quick step",
            explanation:
              "Conflates the light-dependent ATP/NADPH generating stage with the stroma Calvin cycle synthesis stage.",
            hint: "Remind Alex that light energy must first charge chemical batteries before sugar can be built.",
            correctedSnippet:
              "generate ATP and NADPH batteries first, which then power the Calvin cycle to build glucose",
          },
          {
            id: "misc-2",
            ...
            explanation:
              "Water is consumed (split) to provide replacement electrons, releasing Oxygen (O2) gas, not water.",
            hint: "Water is the electron donor that gets split, what gas is released into the air?",
            correctedSnippet:
              "splitting water molecules to release Oxygen gas into the atmosphere",
          },
        ],
        challengeQuestion:
          "If chlorophyll makes glucose right away when light hits it, why do plants need to absorb water at the roots?",
        learningObjectives: [
      ...
      turns: [
        {
          ...
          turnIndex: 1,
          userExplanation:
            "Light does not make glucose instantly. First, photons strike chlorophyll in the thylakoid membrane, which splits water molecules to generate ATP and NADPH energy batteries. Then those batteries power the Calvin cycle to assemble carbon dioxide into glucose!",
          apprenticeReaction:
            "Aha! So light is just charging temporary batteries (ATP & NADPH) by splitting water? That explains why oxygen bubbles out!",
          emotion: "lightbulb",
          delta: 48,
          understandingScore: 68,
          solvedPoints: [
            "Two-Stage Separation (Light vs Calvin Cycle)",
            "Water Photolysis byproduct is Oxygen",
          ],
          timestamp: new Date().toISOString(),
        },
      ],
    ...
    localStorage.setItem(
      "learnora_feynman_sessions",
      JSON.stringify([mockFeynman]),
    );
    ...
    const mockPreMortem = {
      ...
      radarData: [
        ...
        {
          topic: "Taylor Series Convergence",
    ...
    localStorage.setItem(
      "learnora_premortem_reports_v1",
      JSON.stringify([mockPreMortem]),
    );
  });
  ...
  await page.goto("http://localhost:5199/app/debugger", {
    waitUntil: "networkidle",
  });
  await page.click('[data-testid="preset-btn-0"]');
  await page.click('[data-testid="diagnose-submit-btn"]');
  await page.waitForSelector('[data-testid="knowledge-circuit"]', {
    timeout: 8000,
  });
  ...
  await page.waitForSelector('[data-testid="repair-timer-display"]', {
    timeout: 8000,
  });
  ...
  await page.goto("http://localhost:5199/app/feynman", {
    waitUntil: "networkidle",
  });
  ...
  await page.goto(
    "http://localhost:5199/app/feynman/studio/feynman-session-demo",
    { waitUntil: "networkidle" },
  );
  await page.waitForSelector('[data-testid="understanding-gauge"]', {
    timeout: 8000,
  });
  ...
  await page.goto("http://localhost:5199/app/premortem", {
    waitUntil: "networkidle",
  });
  ...
  await page.goto("http://localhost:5199/app/premortem/radar", {
    waitUntil: "networkidle",
  });
  await page.waitForSelector('svg[aria-label="SVG radar spiderweb chart"]', {
    timeout: 8000,
  });
  await save("06_premortem_failure_radar.png");

  // 7. Socratic Sparring
  console.log("Capturing Socratic Sparring...");
  await page.goto("http://localhost:5199/app/sparring", {
    waitUntil: "networkidle",
  });
  await page.waitForTimeout(600);
  await save("07_socratic_sparring.png");

  // 8. Adaptive Weekly Plan
  console.log("Capturing Adaptive Weekly Plan...");
  await page.goto("http://localhost:5199/app/plan", {
    waitUntil: "networkidle",
  });
  await page.waitForTimeout(600);
