...
import type { CreateEntityType, OpenCreateModalOptions } from "../../context/createModal";
...
export function CreateModal({ options, onClose }: CreateModalProps) {
  ...
  const [type, setType] = useState<CreateEntityType>(options?.type ?? "material");
  const { title, subtitle } = TYPE_COPY[type];
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} subtitle={subtitle}>
      <div className={shared.inputGroup}>
        ...
        <div className={shared.segmented} role="radiogroup" aria-labelledby="create-type-label">
          {TYPE_ORDER.map((t) => (
      ...
      {type === "material" ? (
        <MaterialPanel folderId={options?.folderId} onClose={onClose} onDone={options?.onDone} />
      ) : null}
      {type === "subject" ? <SubjectPanel onClose={onClose} onDone={options?.onDone} /> : null}
      {type === "exam" ? <ExamPanel onClose={onClose} onDone={options?.onDone} /> : null}
      {type === "task" ? <TaskPanel onClose={onClose} onDone={options?.onDone} /> : null}
    </Modal>
  );
}
