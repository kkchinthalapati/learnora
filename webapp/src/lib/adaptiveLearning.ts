...
export function getAdaptiveRecommendations(params: {
  ...
  if (retention.overdueCount > 0) {
    recommendations.push({
      ...
      title: `${retention.overdueCount} flashcard${retention.overdueCount > 1 ? "s are" : " is"} due`,
      description: `Go through them now and ${retention.criticalCardsCount} card${retention.criticalCardsCount === 1 ? "" : "s"} you're about to forget will stick.`,
  ...
  if (quizAttempts.length > 0) {
    ...
    if (lowScoreAttempts.length > 0) {
      recommendations.push({
        ...
        category: "quiz_drill",
        priority: "medium",
        title: "Work on your weaker quiz topics",
        description:
          "Your recent quiz scores have some room in them. Ten quick questions will help.",
  ...
  if (pendingTasks.length > 0) {
    recommendations.push({
      ...
      title: `${pendingTasks.length} task${pendingTasks.length > 1 ? "s" : ""} to do`,
      description: `Get "${pendingTasks[0].text}" out of the way in one focused block.`,
      actionUrl: "/tasks",
  ...
  if (health.burnoutRisk === "high") {
    recommendations.unshift({
      ...
      priority: "high",
      title: "Time for a proper break",
      description:
        "You've been at it hard lately. Take 15 minutes away from it.",
      actionUrl: "/timer",
  ...
  if (folders.length > 1 && sessions.length > 0) {
    ...
    if (understudied) {
      recommendations.push({
        ...
        category: "focus_sprint",
        priority: "low",
        title: `${understudied.folderName} could use some time`,
        description: `Only ${understudied.percentage}% of your recent study has been ${understudied.folderName}.`,
