...
| 19 | S4B2 — dual-write notebook ownership | `api/types.ts`, `api/{materials,decks,quizzes,studyPackage,aiQuiz}.ts`, `views/notebooks/NotebookStudioView.tsx` + test | Extended the three legacy Library row shapes with transitional `notebook_id` and taught every creation primitive to accept an optional notebook owner without changing legacy callers. Notebook Studio now supplies its own id when generating a deck, generating a quiz, or exporting a cheat-sheet artifact; each row still receives `folder_id` for the existing subject and analytics screens. The shared study-package pipeline also propagates an optional notebook id through source material and both generated outputs, ready for the unified Create flow. Optional fields are omitted, rather than written as null, for callers outside a notebook. | `npx vitest run src/views/notebooks/NotebookStudioView.test.tsx src/api/studyPackage.test.ts src/api/materials.test.ts src/api/quizzes.test.ts --pool=threads --maxWorkers=1 --no-file-parallelism` → **4/4 files, 57/57 tests passed** after adding the artifact-export mutation check. `npm run build` → **✓ built in 5.56s**. `git diff --check` → clean. | done |
| 20 | S4C2 — deterministic legacy-content backfill | `supabase/migrations/20260909020000_backfill_library_notebooks.sql` | Added an internal `system_key`, identified by partial unique indexes rather than user-visible titles. The migration creates one compatibility notebook for every existing folder and a per-user `Unfiled sources` notebook only for accounts with genuinely unfiled material/decks/quizzes. It assigns only rows whose `notebook_id` is null, so ownership already written by the application is never overwritten. A student-created notebook called "Unfiled sources" is left alone because title matching is never used. | PowerShell structural check → **3 content-table updates and 3 `notebook_id is null` overwrite guards**; both compatibility markers occur in their insert, uniqueness and assignment paths. `git diff --check` → clean. SQL execution remains unverified because the local Docker daemon is unavailable; no remote environment was touched. | done |
| 21 | S4C3 — keep subject notebooks in sync | `supabase/migrations/20260909030000_sync_folder_notebooks.sql` | Added an `AFTER INSERT OR UPDATE OF name, color` folder trigger that upserts the folder's internal `folder_contents` notebook. New subjects therefore enter the unified model after the one-time backfill, and renaming or recolouring a subject cannot leave its compatibility notebook stale. The function is `SECURITY DEFINER` with an empty search path and direct execution is revoked from public/API roles. Folder deletion remains governed by the Stage 4 `ON DELETE CASCADE` foreign key. | PowerShell structural check → **1 security-definer declaration, 1 empty search path, 3 API-role revokes, 1 insert/update trigger**. `git diff --check` → clean. SQL execution remains unverified because the local Docker daemon is unavailable; no remote environment was touched. | done |
| 22 | S4C4 — enforce new-row notebook ownership | `supabase/migrations/20260909040000_assign_library_notebook_ownership.sql` | Made notebook ownership a database invariant for new materials, decks and quizzes, including callers that have not yet been migrated to pass `notebook_id`. A shared `BEFORE INSERT` trigger preserves explicit student-notebook ownership; otherwise it deterministically assigns the folder compatibility notebook or creates the account's internal unfiled notebook. It never matches user-visible titles. With the prior backfill and future inserts both covered, all three columns are now `NOT NULL`. The trigger function is `SECURITY DEFINER` with an empty search path and direct execution revoked from API roles; existing restrictive policies still validate the row and resolved parent against the authenticated owner. | PowerShell structural check → **3 ownership triggers, 3 `NOT NULL` transitions, 3 API-role revokes, 2 partial-index upserts, 1 explicit-owner preservation guard and 1 empty search path**. `git diff --check` → clean. SQL execution remains unverified because the local Docker daemon is unavailable; no remote environment was touched. | done |
